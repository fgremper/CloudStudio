				            ...
				            while (treeWalk.next()) {
				            	if (treeWalk.getPathString().equals(filename)) {
					            	...
					            	diff3.diffFile(
					            			...
					            			fileStorageDirectory + "/" + ancestorSha,
					            			fileStorageDirectory + "/" + db.getFileSha(repositoryAlias, theirUsername, compareToBranch, filename, showUncommitted)
					            		);

					        		Vector<Hunk3> vector = (Vector<Hunk3>) diff3.getHunk3();
					        		//diff3.toString();
